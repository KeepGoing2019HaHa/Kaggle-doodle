...
# 2nd cls
# CLASS_NAME = ['broccoli', 'tree']
# CLASS_NAME = ['hurricane', 'tornado']
# CLASS_NAME = ['school_bus', 'bus']
# CLASS_NAME = ['cake', 'birthday_cake']
# CLASS_NAME = ['coffee_cup', 'mug']
...
def drawing_to_image(drawing, H, W):
    ...
    # norm_point = (norm_point-[w/s*0.5,h/s*0.5])*max(W,H)*0.98
    # norm_point = (norm_point-[w/s*0.5,h/s*0.5])*max(W,H)*0.90
...
# # stroke2image from discuss
# def drawing_to_image(raw_strokes, H, W, lw=6, time_color=True):
#
#     BASE_SIZE = 256
...
class ReRankingDataset(Dataset):
    def __init__(self, re_ranking_list, augment):
        super(ReRankingDataset, self).__init__()
        self.df = pd.read_csv('/coer/yiwei/kaggle/doodle/data/csv/test_simplified.csv')
        self.re_ranking_list=re_ranking_list
    ...
    def __getitem__(self, index):
        label=None
        drawing = self.df['drawing'][self.re_ranking_list[index]]
        drawing = eval(drawing)
        return self.augment(drawing, label, index)

    def __len__(self):
        return len(self.re_ranking_list)
        ...
        if mode=='train':
            ...
            if TRAIN_DF == []:
                for l,name in enumerate(CLASS_NAME):
                    ...
                    # df = pd.read_csv(DATA_DIR + '/csv/train_%s/%s.csv'%(complexity,name), nrows=10000)
...
def run_check_train_data():
    ...
    for m in range(num):
        ...
        cv2.imwrite('/tmp/doodle/%d.jpg'%m, image)
        if m>10: break
        # overlay=255-image
